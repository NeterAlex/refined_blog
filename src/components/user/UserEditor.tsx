...
import {Avatar, Button, Card, Center, Container, Divider, FileInput, Group, Image, Paper, rem, SimpleGrid, Text, TextInput, useMantineTheme} from "@mantine/core";
...
export default function UserEditor(props: Props) {
    ...
    const form = useForm({
        initialValues: {nickname: props.nickname, email: props.email},
        validate: {
            nickname: isNotEmpty('昵称不应为空'),
            email: isEmail('请输入正确的邮箱地址'),
        }
    })
    const {trigTokenUpdate} = useTokenUpdate(`/user/${props.uid}`, form.values, {title: '用户信息更新成功', message: '将返回首页', color: 'green'})
    return (
        <Container mt={'xl'}>
            <Card withBorder shadow="sm" mt={30} radius="md">
                ...
                <SimpleGrid cols={2} spacing="md" breakpoints={[{maxWidth: 'sm', cols: 1}]}>
                    <Paper withBorder p={30} mt={30} radius="md" component="form" onSubmit={async () => {
                        ...
                        await axios.post(`/api/v1/file/avatar/${props.uid}`, formData, {
                                headers: {
